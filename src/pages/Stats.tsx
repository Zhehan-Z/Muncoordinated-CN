...
export default class Stats extends React.Component<Props, State> {
  ...
  memberStats(committee: CommitteeData, memberID: MemberID, member: MemberData): MemberStats {
    ...
    Object.keys(caucuses).forEach((cid) => {
      ...
      Object.keys(history)
        .map((hid) => history[hid])
        .forEach((speakerEvent: SpeakerEvent) => {
          if (speakerEvent.who === member.name) {
            // I fucked up and used name in SpeakerEvent, not MemberID
            times += 1;
    ...
    Object.keys(resolutions).forEach((rid) => {
      ...
      Object.keys(amendments)
        .map((aid) => amendments[aid])
        .forEach((amendment: AmendmentData) => {
