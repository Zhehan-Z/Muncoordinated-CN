...
import {
  checkboxHandler,
  clearableZeroableValidatedNumberFieldHandler,
  fieldHandler,
...
export function DeleteStrawpollModal(props: ModalProps) {
  ...
  return (
    ...
    >
      ...
      <Modal.Content>
        <Modal.Description>Are you sure that you want to delete this strawpoll?</Modal.Description>
      </Modal.Content>
      <Modal.Actions>
...
export default function Strawpoll(props: StrawpollProps) {
  ...
  const type: StrawpollType = strawpoll
    ? strawpoll.type || StrawpollType.Checkbox
    : StrawpollType.Checkbox;
  const stage: StrawpollStage = strawpoll
    ? strawpoll.stage || StrawpollStage.Preparing
    : StrawpollStage.Preparing;
  const medium: StrawpollMedium = strawpoll
    ? strawpoll.medium || StrawpollMedium.Link
    : StrawpollMedium.Link;
  const options: Record<StrawpollOptionID, StrawpollOptionData> = strawpoll
    ? strawpoll.options || {}
    : {};
  ...
  const onCheck =
    (oid: StrawpollOptionID) => (event: React.FormEvent<HTMLInputElement>, data: CheckboxProps) => {
      if (type === StrawpollType.Checkbox) {
        if (data.checked) {
          strawpollFref.child('options').child(oid).child('votes').child(voterID).set(true);
        } else {
          strawpollFref.child('options').child(oid).child('votes').child(voterID).remove();
        }
      } else if (type === StrawpollType.Radio) {
        if (data.checked) {
          // Set everything to unchecked
          Object.keys(options).forEach((id) =>
            strawpollFref.child('options').child(id).child('votes').child(voterID).remove()
          );
          strawpollFref.child('options').child(oid).child('votes').child(voterID).set(true);
        }
      }
  ...
  const renderOption = (optionID: StrawpollOptionID, option: StrawpollOptionData) => {
    const strawpollOptionFref = strawpollFref.child('options').child(optionID);

    const isChecked = option.votes
      ...
      case StrawpollStage.Preparing:
        return (
          <List.Item key={optionID}>
            ...
            >
              <input />
              <Button negative icon="trash" basic onClick={() => strawpollOptionFref.remove()} />
            </Input>
          </List.Item>
        );
        ...
        return medium === StrawpollMedium.Link ? (
          ...
          <List.Item key={optionID}>
            <Input
              ...
              onChange={clearableZeroableValidatedNumberFieldHandler<StrawpollOptionData>(
                strawpollOptionFref,
                'tally'
              )}
            />
    ...
    return (
      <List>
        ...
        {stage === StrawpollStage.Preparing && (
          <List.Item>
            <Button basic fluid onClick={addOption}>
              <Icon name="plus" />
              Add option
            </Button>
          </List.Item>
            ...
            <List.Item>
              <Button.Group fluid>
                ...
                <DeleteStrawpollModal
                  ...
                  trigger={
                    <Button color="red" basic onClick={() => setOpen(true)}>
                      <Icon name="delete" />
                      Delete strawpoll?
                    </Button>
                  }
  ...
  const renderNavButtons = () => {
    switch (stage) {
      case StrawpollStage.Preparing:
        return (
          <Button.Group fluid>
            <Button primary basic onClick={createSharablePoll}>
              Create shareable poll
              <Icon name="arrow right" />
            </Button>
            <Button.Or />
            <Button primary basic onClick={createManualPoll}>
              Create manual poll
              <Icon name="arrow right" />
            </Button>
          </Button.Group>
      ...
      case StrawpollStage.Voting:
        return (
          <Button.Group fluid>
            <Button basic secondary onClick={editOptions}>
              <Icon name="arrow left" />
              Edit options
            </Button>
            <Button primary basic disabled={!user} onClick={viewResults}>
              View results
              <Icon name="arrow right" />
            </Button>
          </Button.Group>
      ...
      case StrawpollStage.Results:
        return (
          <Button fluid secondary basic onClick={reopenVoting}>
            <Icon name="arrow left" />
            Reopen voting
          </Button>
        );
