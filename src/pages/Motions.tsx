...
const DIVISIBILITY_ERROR = (
  <Message error content="Speaker time does not evenly divide the caucus time" />
);
...
export class MotionsComponent extends React.Component<Props & Hooks, State> {
  ...
  handleApproveMotion = (motionFref: firebase.database.Reference, motionData: MotionData): void => {
    ...
    if (
      ...
      this.props.history.push(`/committees/${committeeID}/caucuses/${caucusRef.key}`);
    } else if (
      ...
      this.props.history.push(`/committees/${committeeID}/resolutions/${resolutionRef.key}`);
      ...
      this.props.history.push(`/committees/${committeeID}/resolutions/${resolutionID}/amendments`);
      ...
      this.props.history.push(`/committees/${committeeID}/resolutions/${resolutionID}/voting`);
      ...
      this.props.history.push(`/committees/${committeeID}/strawpolls/${strawpollRef.key}`);
    }
  };
        ...
        <Button.Group className="thirdwidth">
          <Popup
            ...
            trigger={
              ...
              >
                <Icon
                  name={
                    votes[voterID] === MotionVote.Against ? 'thumbs down' : 'thumbs down outline'
                  }
                />
          ...
          {procedural(motionData.type) && (
            <Popup
              ...
              trigger={
                ...
                >
                  <Icon
                    name={votes[voterID] === MotionVote.Abstain ? 'circle' : 'circle outline'}
                  />
          ...
          <Popup
            ...
            trigger={
              ...
              >
                <Icon
                  name={votes[voterID] === MotionVote.For ? 'thumbs up' : 'thumbs up outline'}
                />
    ...
    return (
      <Container text style={{ padding: '1em 0em' }}>
        ...
        <Checkbox
          ...
          toggle
          checked={motionVotes}
          onChange={checkboxHandler<SettingsData>(committeeFref.child('settings'), 'motionVotes')}
        />
        ...
        <Divider />
        <Icon name="sort numeric ascending" /> Sorted from most to least disruptive. {operative}{' '}
